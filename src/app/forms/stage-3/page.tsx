...
const TRANSLATIONS = {
  en: {
    title: "Stage 3 — Product Details",
    intro: "Final stage. Enter your email and product information.",
    email: "Email Address", product: "Product Type", quantity: "Production Quantity (per month)",
    submit: "Submit Application", footer: "Encrypted · Response within 5 business days",
  },
  zh: {
    title: "第3阶段 — 产品详情",
    intro: "最后阶段。输入您的电子邮件和产品信息。",
    email: "电子邮件地址", product: "产品类型", quantity: "生产数量(每月)",
...
export default function Stage3() {
  ...
  const [form, setForm] = useState({ email: "", product_type: "", production_quantity: "" });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/forms/stage-3", {
    ...
    <div className="min-h-screen flex items-center justify-center p-8 bg-bg-base">
      <div className="w-full max-w-md bg-white border border-border rounded-2xl p-8 shadow-sm relative">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div><label className="label">{t.product}</label>
            <input className="input" required value={form.product_type} onChange={(e) => setForm({ ...form, product_type: e.target.value })} /></div>
          <div><label className="label">{t.quantity}</label>
            <input className="input" required value={form.production_quantity} onChange={(e) => setForm({ ...form, production_quantity: e.target.value })} /></div>
